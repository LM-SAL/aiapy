...
We will walk you through setting up a working installation of Python and ``aiapy``.
For further information and alternative methods for installing ``aiapy`` beyond the recommended approach outlined below, refer to sunpy's documentation (:ref:`sunpy-topic-guide-installing`).
...
As well as containing a Python installation, this virtual environment provides an isolated place to install Python packages (like ``aiapy``) without affecting any other current Python installation.
...
Miniforge will install ``conda`` and automatically configure the default channel (a channel is a remote software repository) to be ``conda-forge``, which is where ``aiapy`` is available.
...
Now we will create and activate a new virtual environment to install ``aiapy`` into:
...
In this case the virtual environment is named 'aiapy'.
...
Now that we have a fresh virtual environment, we can proceed with installing ``aiapy``:
...
If you are planning on using aiapy in jupyter notebooks we also recommend you install the ``ipywidgets`` and ``itables`` packages.

To ensure that ``aiapy`` was installed correctly, run the following command:

.. code-block:: bash

    $ conda list aiapy

This checks if ``aiapy`` was installed correctly.

If you want to install another package later, you can run ``conda install <package_name>``.

If you want the optional performance dependencies installed, you can do the following:
