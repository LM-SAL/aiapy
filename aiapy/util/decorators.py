...
_all_channels = [94*u.angstrom, 131*u.angstrom, 171*u.angstrom, 193*u.angstrom,
...
def validate_channel(argument, valid_channels='all'):
    ...
    valid_channels : {'all'}, list
        List of valid channels. If ``'all'``, validate against the list of all
        AIA channels.
    """
    ...
    def outer(function):
        ...
        if argument not in sig.parameters:
            raise ValueError(f'Did not find {argument} in '
                             f'function signature ({params}).')
        ...
        def inner(*args, **kwargs):
            ...
            if channel not in valid_channels:
                raise ValueError(f'channel "{channel}" not in '
