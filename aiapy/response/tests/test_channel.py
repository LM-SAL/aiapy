...
def test_invalid_channel():
    with pytest.raises(
        ValueError, match='channel "1.0 Angstrom" not in ' "list of valid channels"
    ):
...
def test_wavelength_response_uncorrected(channel, idl_environment):
    ...
    ssw = idl_environment.run(
        "r = aia_get_response(/area,/dn,evenorm=0)", save_vars=["r"], verbose=False
    )
...
@pytest.mark.parametrize(
    "channel_wavelength", [1600 * u.angstrom, 1700 * u.angstrom, 4500 * u.angstrom]
)
