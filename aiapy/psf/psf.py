...
@u.quantity_input
@validate_channel(
    "channel", valid_channels=[94, 131, 171, 193, 211, 304, 335] * u.angstrom
)
def psf(
    channel: u.angstrom, use_preflightcore=False, diffraction_orders=None, use_gpu=True
):
    r"""
...
def _psf(meshinfo, angles, diffraction_orders, focal_plane=False, use_gpu=True):
    ...
    for order in diffraction_orders:
        ...
        for dx, dy in zip(spacing_x.value, spacing_y.value):
            ...
            psf += (
                np.exp(
                    -width_x * x_centered * x_centered
                    - width_y * y_centered * y_centered
                )
                * intensity
            )
    # Contribution from core
    psf_core = np.exp(
        -width_x * (x - 0.5 * Nx - 0.5) ** 2 - width_y * (y - 0.5 * Ny - 0.5) ** 2
    )
    psf_total = (
        1 - area_not_mesh
    ) * psf / psf.sum() + area_not_mesh * psf_core / psf_core.sum()
    return psf_total
