...
def test_register_unsupported_maps(aia_171_map):
    ...
    """
    # A submap
    original_cutout = aia_171_map.submap(
        aia_171_map.center, top_right=aia_171_map.top_right_coord
    )
    with pytest.raises(ValueError):
        _ = register(original_cutout)
    # A Map besides AIA or HMI
    non_sdo_map = Map(
        sunpy.data.test.get_test_filepath("swap_lv1_20140606_000113.fits")
    )
    with pytest.raises(ValueError):
...
def test_degradation_time_array():
    ...
    for o, tc in zip(obstime, time_correction):
        assert tc == degradation(
            94 * u.angstrom, o, correction_table=correction_table, calibration_version=8
        )
