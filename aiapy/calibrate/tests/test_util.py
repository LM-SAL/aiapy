...
def test_invalid_wavelength_raises_exception():
    with pytest.raises(
        ValueError, match='channel "1800.0 Angstrom" not in list of valid channels'
    ):
...
def test_wrong_version_number_raises_exception():
    with pytest.raises(
        IndexError, match="Correction table does not contain calibration for version -1"
    ):
...
def test_obstime_out_of_range():
    ...
    with pytest.raises(
        IndexError, match=f"No valid calibration epoch for {obstime_out_of_range}"
    ):
        _select_epoch_from_table(
            94 * u.angstrom, obstime_out_of_range, table_local, version=8
        )
